...
Most of the pipeline is deterministic code: fetching, clustering pre-filters, quote verification, scoring and exposure overlay. We only call Claude for events that pass the corroboration gate, which is a few events per cycle. A full cycle with every event re-briefed is about 40 Claude calls, which comes to under $1 at Sonnet-class pricing. But Claude responses are cached by a hash of the evidence, so an event whose sources haven't changed isn't re-briefed. In steady state that's **a few dollars a day** at a 10-minute cadence, roughly the cost of one analyst hour a month. If the API is down, a deterministic template fallback still produces the alert.
